---
layout: home
title: "Kim Hyeongbeom | Portfolio & Blog"
---

# 👋 Hello! I'm Kim Hyeongbeom.
> **AI Researcher** specializing in Change Captioning | B.S. in Life Sciences & EECS

[GitHub](https://github.com/hbk08101) | [LinkedIn](https://www.linkedin.com/in/%ED%98%95%EB%B2%94-%EA%B9%80-328574410/)
📧 081007a@gmail.com & hbk08101@gm.gist.ac.kr | 📍 Gwangju, South Korea

---

## 🎓 Education

### **Gwangju Institute of Science and Technology (GIST)**
*Feb 2019 – Aug 2025*
* **B.S., Double Major:** Life Sciences and Electrical Engineering & Computer Science
* **Minor:** AI Convergence
* **GPA:** 4.02 / 4.5 *(Graduated with Honors)*

### **Gwangju Science Academy**
*Mar 2016 – Jan 2019*
* **Highlights:** Graduated; Outstanding R&E Grand Prize (Environmental Purification Project)
* **Leadership:** Student Council Treasurer

---

## 🔬 Experience

### **Research Staff** | ACSL Lab @ GIST
*Aug 2025 – Present*
* Conducted research on Change Captioning model benchmarks.
* Managed and performed extensive data collection and labeling tasks.

### **Research Intern** | ACSL Lab @ GIST
*Sep 2024 – Aug 2025*
* Conducted research on improving and extending Change Captioning methods.

---

## 📚 Publications

* 📄 **[C3-Bench: A Context-Aware Change Captioning Benchmark]**
  * *Co-author* | Published at **ECCV 2026**

---

## 💡 Tech Stack & Interests

| Category | Details |
| :--- | :--- |
| **Languages** | `Python` `C` `Java` `Bash` |
| **AI & ML** | `AI Modeling` `Change Captioning` |

---

## 📜 Certifications & Languages

* **SQL Developer (SQLD)** | *Jun 19, 2026*
* **Korean History Proficiency Test Level 1** (한국사능력검정시험 1급) | *Jun 5, 2026*
* **TOEIC:** 870 | *Mar 15, 2026*
